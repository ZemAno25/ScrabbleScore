...
class Scorer {
    ...
    public function placeAndScore(string $pos, string $word): PlacementResult {
        ...
        foreach ($letters as $tok) {
            if ($tok['type'] === 'exist') {
                ...
                if ($this->board->cells[$r][$c]['letter'] === null) {
                    throw new RuntimeException("Płytka '{$tok['char']}' w nawiasie wymaga, aby pole " . Board::xyToCoord($r,$c) . " było już zajęte.");
                }
                $coords[] = [$r,$c,false];
                $r += $dr; $c += $dc;
                continue;
            ...
            // placing a new tile
            if ($this->board->cells[$r][$c]['letter'] !== null) {
                throw new RuntimeException("Pole " . Board::xyToCoord($r,$c) . " jest już zajęte.");
            }
        ...
        foreach ($coords as $info) {
            ...
            if ($isNew) {
                $ch = $info[3];
                $isBlank = $info[4];
                $this->board->cells[$rr][$cc] = ['letter'=>$ch,'isBlank'=>$isBlank,'locked'=>false];
                $res->placed[] = [$rr,$cc];
                $res->lettersPlaced++;
            }
        }
        ...
        foreach ($res->placed as [$pr,$pc]) {
            $adr = $dc; $adc = $dr; // perpendicular
            $start = $this->findWordStart($pr,$pc,$adr,$adc);
            $len = $this->wordLen($start[0],$start[1],$adr,$adc);
            if ($len > 1) {
                $res->score += $this->scoreWordLine($start[0],$start[1],$adr,$adc);
            }
        }
        ...
        // Bingo
        if ($res->lettersPlaced === 7) $res->score += 50;
        return $res;
    }
    ...
    private function normalize(string $w): string {
        ...
        for ($i=0; $i<$len; $i++) {
            ...
            if ($ch === $up) { $out .= $up; } else { $out .= $ch; } // keep lowercase
        }
        return $out;
    }
    ...
    private function tokenize(string $w): array {
        ...
        for ($i=0;$i<$len;$i++){
            ...
            if ($inPar) {
                $tokens[] = ['type'=>'exist','char'=>mb_strtoupper($ch,'UTF-8')];
            } else {
                $isBlank = (mb_strtolower($ch,'UTF-8') === $ch);
                $tokens[] = ['type'=>'new','char'=>mb_strtoupper($ch,'UTF-8'),'isBlank'=>$isBlank];
            }
        }
        return $tokens;
    }
    ...
    private function findWordStart(int $r,int $c,int $dr,int $dc): array {
        while ($r-$dr>=0 && $c-$dc>=0 && $r-$dr<15 && $c-$dc<15 && $this->board->cells[$r-$dr][$c-$dc]['letter'] !== null) {
            $r -= $dr; $c -= $dc;
        }
        return [$r,$c];
    }
    ...
    private function wordLen(int $r,int $c,int $dr,int $dc): int {
        $len=0;
        while ($r>=0 && $c>=0 && $r<15 && $c<15 && $this->board->cells[$r][$c]['letter'] !== null) {
            $len++; $r+=$dr; $c+=$dc;
        }
        return $len;
    }
    ...
    private function scoreWordLine(int $row,int $col,int $dr,int $dc, ?array &$coordsOut=null): int {
        // find start
        ...
        if ($coordsOut !== null) $coordsOut = $coords;
        return $sum * $wm;
    }
}
